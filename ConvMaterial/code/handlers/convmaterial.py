...
class ConvMaterialHandler(BaseHandler):
    # 数据库连接初始化
    def initialize(self):
        self.client = pymongo.MongoClient(**config['mongo'])
        self.db = self.client[config['db']['mes_db_name']]
        self.coll = self.db[config['coll']['convmaterial_coll_name']]
    ...
    def get(self):
        try:
            if self.get_argument('date', ''):
                query = {
                    "_id": self.get_argument('date')
                }
                data = {
                    "state": "success",
                    "message": "查询成功！",
                    "payload": self.coll.find_one(query)
                }
                ...
                data = {
                    "state": "success",
                    "message": "查询成功！",
                    "payload":  [x for x in self.coll.find({})]
                }
                self.write(json.dumps(data))
        except:
            data = {
