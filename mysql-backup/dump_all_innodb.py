...
# 消息主题
subject = 'MySQL数据库全备(InnoDB)'
# 用户账号
user = ''
# 企业号的标识
corp_id = 'xxxxxxxxxxxxxx'
# 管理组凭证密钥
secret = 'xxxxxxxxxxxx-xxxxxxxxxxxxxxxxxx'
# 通讯录标签ID
tag_id = ''
# 应用ID
agent_id = '1000002'
...
def main():
    ...
    file_name = 'all_innodb_{}.sql'.format(date_time)
    file_fullname = os.path.join(gen_dir(backup_path), file_name)
    cmd = 'mysqldump --set-gtid-purged=OFF --single-transaction --flush-logs --master-data=2 --all-databases --triggers --routines --events > {}'.format(
        file_fullname)
    print('---备份信息---')
    rc = os.system(cmd)
    if rc == 0:
