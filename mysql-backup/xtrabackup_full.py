...
# 管理组凭证密钥
secret = 'xxxxxxxxxx-xxxxxxxxxxxxxxxxxxxxx'
# 通讯录标签ID
tag_id = ''
# 应用ID
agent_id = '1000002'
...
def main():
    ...
    # 执行全库备份之前先清空备份文件夹
    clean_dir(backup_path)
    date_time = get_date_time('%Y%m%d_%H%M%S')
    dir_name = 'full_{}'.format(date_time)
    dir_fullname = os.path.join(gen_dir(backup_path), dir_name)
    cmd = 'xtrabackup --backup --target-dir={} --datadir={}/'.format(
        gen_dir(dir_fullname), mysql_datadir)
    print('---备份信息---')
    rc = os.system(cmd)
    if rc == 0:
        # 记录最新备份，用于增量备份
