...
# 消息主题
subject = 'MySQL数据库单库备份(InnoDB)'
# 用户账号
user = ''
# 企业号的标识
corp_id = 'xxxxxxxxxxxxxxxx'
# 管理组凭证密钥
secret = 'xxxxxxxxxxx-xxxxxxxxxxxxxxxxxxxx'
# 通讯录标签ID
tag_id = ''
# 应用ID
agent_id = '1000002'
...
def main(db_name):
    """单库备份(InnoDB)"""
    if db_name in get_all_db():
        print('***备份开始***')
        begin_datetime = get_date_time('%Y-%m-%d %H:%M:%S')
        print('开始时间：' + begin_datetime)
        begin_time = datetime.datetime.now()
        file_name = '{}_innodb_{}.sql'.format(
            db_name, get_date_time('%Y%m%d_%H%M%S'))
        file_fullname = os.path.join(gen_dir(backup_path), file_name)
        cmd = 'mysqldump --set-gtid-purged=OFF --single-transaction {} > {}'.format(
            db_name, file_fullname)
        print('---备份信息---')
        rc = os.system(cmd)
        if rc == 0:
...
if __name__ == "__main__":
    ...
    if len(arg_list) == 2:
        ...
        print('option error, please input a `dbname` option.')
