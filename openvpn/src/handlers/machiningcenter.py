...
class MachiningCenterHandler(BaseHandler):
    # 数据库连接初始化
    def initialize(self):
        self.client = pymongo.MongoClient(**config.mongodb_options)
        self.db = self.client[config.db['tlink_db_name']]
        self.coll = self.db[config.coll['machiningcenter_coll_name']]

    # 查询加工中心信息
    def get(self):
        #try:
           if self.get_argument('MachiningCenterID',''):
               query = {
                   "MachiningCenterID": self.get_argument('MachiningCenterID')
               }
               data = {
                   "state": "success",
                   "message": "查询成功！",
    ...
    def post(self):
        #try:
           ...
           query = {
               "MachiningCenterID": request_data.get('MachiningCenterID')
           }
