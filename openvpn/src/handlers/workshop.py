...
class WorkshopHandler(BaseHandler):
    # 数据库连接初始化
    def initialize(self):
        self.client = pymongo.MongoClient(**config.mongodb_options)
        self.db = self.client[config.db['tlink_db_name']]
        self.coll = self.db[config.coll['workshop_coll_name']]

    # 查询车间信息
    def get(self):
        #try:
           if self.get_argument('WorkshopID',''):
               query = {
                   "WorkshopID": self.get_argument('WorkshopID')
               }
               data = {
                   "state": "success",
                   "message": "查询成功！",
    ...
    def post(self):
        #try:
           ...
           query = {
               "WorkshopID": request_data.get('WorkshopID')
           }
