...
class VehicleStorageHandler(BaseHandler):
    # 数据库连接初始化
    def initialize(self):
        self.client = pymongo.MongoClient(**config['mongo'])
        self.db = self.client[config['db']['mes_db_name']]
        self.coll = self.db[config['coll']['vehiclestorage_coll_name']]

    # 查询信息
    def get(self):
        try:
            if self.get_argument('VehicleStorageID', ''):
                query = {
                    "VehicleStorageID": self.get_argument('VehicleStorageID')
                }
                data = {
                    "state": "success",
                    "message": "查询成功！",
    ...
    def post(self):
        try:
            ...
            query = {
                "VehicleStorageID": request_data.get('VehicleStorageID')
            }
