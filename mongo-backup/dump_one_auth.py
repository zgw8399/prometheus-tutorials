...
def main(db_name):
    ...
    if db_name in db_list:
        ...
        backup_path = os.path.join(backup_root_path, db_name)
        file_name = '{}.{}.gz'.format(db_name, get_date_time('%Y%m%d_%H%M%S'))
        file_fullname = os.path.join(gen_dir(backup_path), file_name)
        cmd = 'mongodump --host={} --port={} --username={} --password={} --authenticationDatabase=admin --archive={} --gzip --db={}'.format(db_host, db_port, db_user, db_password, file_fullname, db_name)
        print('---备份信息---')
        rc = os.system(cmd)
        if rc == 0:
