...
# 管理组凭证密钥
secret = 'xxxxxxxxxx-xxxxxxxxxxxxxxxx'
# 通讯录标签ID
tag_id = ''
# 应用ID
agent_id = '1000002'
...
def main(db_name):
    """单库备份"""
    client = pymongo.MongoClient("mongodb://{}:{}/".format(db_host, db_port))
    ...
    if db_name in db_list:
        ...
        file_name = '{}_{}.gz'.format(db_name, get_date_time('%Y%m%d_%H%M%S'))
        file_fullname = os.path.join(gen_dir(backup_path), file_name)
        cmd = 'mongodump --archive={} --gzip --db={}'.format(
            file_fullname, db_name)
        print('---备份信息---')
        rc = os.system(cmd)
        if rc == 0:
